...
async function downloadEnterleave() {
  const url =
    'http://berrytrials.me:4180/cod4/mods/berry_promod/enterleave.log';
...
async function downloadServerLog() {
  const url =
    'http://berrytrials.me:4180/cod4/mods/berry_promod/server_log.txt';
  ...
  lr.on('line', function (line) {
    ...
    if (i >= 0) {
      ...
      if (firstHalfSplitColon.length >= 2) {
        var firstHalfSplit = firstHalfSplitColon[
          firstHalfSplitColon.length - 2
        ].split(' ');
  ...
  lr.on('end', function () {
    ...
    lr.on('end', function () {
      ...
      lr.on('line', function (line) {
        ...
        if (
          line.includes(' say;') ||
          line.includes(' say_team;') ||
          line.includes(' say_special;')
        ) {
          var msgType = 'say';
          ...
          try {
            ...
            var timeStamp =
              Number(moment().format('x')) + Number(messages.length) + '';
            var inServerBuids = '';
            for (var i = 0; i < activePlayers.length; i++) {
...
function cleanActivePlayersTable(activePlayers) {
  ...
  db.query(query, (err, result) => {
    if (err) {
      console.log(
        'ERROR: Cannot delete players from active_players table',
        err
      );
    } else {
...
function updateChatlog(messages) {
  for (var i = 0; i < messages.length; i++) {
    var m = messages[i];
    var message = db.escape(m.message);
    var name = db.escape(m.name);
    if (message === 'NULL' || name === 'NULL') {
      continue;
    }
    var query =
      'INSERT INTO `chatlog` (`message`, `buid`, `guid`, `name`, `occurred`, `in_server`, `type`, `hash`)' +
      'VALUES (' +
      message +
      ", '" +
      m.buid +
...
function updateServerRuns(runs) {
  for (var i = 0; i < runs.length; i++) {
    var r = runs[i];
    var query =
      'INSERT INTO `server_runs` (`uniq_id`, `buid`, `name`, `zone`, `time`, `occurred`)' +
      "VALUES ('" +
      r.uniqId +
      "', '" +
      r.buid +
...
function updateBerryLog(berryLogs) {
  ...
  for (var i = 0; i < berryLogs.length; i++) {
    var b = berryLogs[i];
    var query =
      'INSERT INTO `berry_log` (`name`, `start_berries`, `end_berries`, `berries`, `operator`, `info`, `bonus`, `buid`, `epochTime`, `strTime`, `hash`)' +
      'VALUES (' +
      db.escape(b.name) +
      ", '" +
      b.start_berries +
...
function updateDatabase(p) {
  ...
  query =
    'INSERT INTO `active_players` (`buid`, `name`, `zone`, `spec`, `afk`, `spec_buid`)' +
    "VALUES ('" +
    p.buid +
    "', '" +
    p.name +
...
function checkBerrySnapshot(p) {
  ...
  var query =
    "SELECT * FROM `berries_snapshot` WHERE buid = '" +
    p.buid +
...
function addBerrySnapshot(p) {
  ...
  var query =
    'INSERT INTO `berries_snapshot` (`buid`, `name`, `berries`, `occurred`)' +
    "VALUES ('" +
    p.buid +
    "', '" +
    p.name +
...
function addRunToDatabase(buid, zone, time, occurred) {
  var query =
    'INSERT INTO `runs` (`buid`, `zone`, `time`, `occurred`) ' +
    "VALUES ('" +
    buid +
    "', '" +
    zone +
